...
OUT = Path(
    os.environ.get(
        "RUNCAT_OUT_FILE",
        str(Path.home() / ".codex" / "runcat-usage.json"),
    )
)
...
def context_metric(token_count):
    ...
    if not isinstance(used_tokens, (int, float)) or not isinstance(
        context_window,
        (int, float),
    ):
        return None
    ...
    effective_used_tokens = max(used_tokens - BASELINE_TOKENS, 0)
    effective_context_window = context_window - BASELINE_TOKENS
    return percentage_metric(
        "Context",
        effective_used_tokens / effective_context_window * 100,
    )
